...
if (testnet && !sidechain) { 
  apiPortDefault = 60507; // signet (testnet)
} else if (!testnet && !sidechain) {
  apiPortDefault = 50507; // signet (mainnet)
...
app.on('ready', () => {
  if (serve) {
    console.log('Signet UI was started in development mode. This requires the user to be running the Signet Full Node Daemon himself.');
  } else {
    if (!nodaemon) {
      startDaemon();
    }
