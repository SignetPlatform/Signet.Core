...
cd $env:BUILD_FOLDER/SignetCore.UI
npm install --verbose
...
cd $env:BUILD_FOLDER/SignetBitcoinFullNode/src/Signet.SignetD
dotnet publish -c $env:configuration -v m -r $env:win_runtime -o $env:BUILD_FOLDER\SignetCore.UI\daemon
if ($LastExitCode -ne 0) { $host.SetShouldExit($LastExitCode)  }
   
Write-Host "Building and packaging SignetCore.UI" -foregroundcolor "magenta"
cd $env:BUILD_FOLDER/SignetCore.UI
npm run package:windows64
if ($LastExitCode -ne 0) { $host.SetShouldExit($LastExitCode)  }     
Write-Host "[$env:configuration][$env:win_runtime] FINISHED SignetCore.UI packaging" -foregroundcolor "magenta"
