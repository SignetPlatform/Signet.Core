...
cd $TRAVIS_BUILD_DIR/SignetCore.UI
...
cd $TRAVIS_BUILD_DIR/SignetBitcoinFullNode/src/Signet.SignetD
dotnet publish -c $configuration -r $TRAVIS_OS_NAME-$arch -v m -o $TRAVIS_BUILD_DIR/SignetCore.UI/daemon

echo $log_prefix chmoding the Signet.SignetD file
chmod +x $TRAVIS_BUILD_DIR/SignetCore.UI/daemon/Signet.SignetD
...
cd $TRAVIS_BUILD_DIR/SignetCore.UI/app-builds/
